...
def plot_energy(method, filedir, n_particles, n_dims, iterations):
    ...
    title = "Energy as a function of gradient descent iteration using " + method_full_name +  " \n with " + str(iterations) + " mc-iterations" + " for " + str(temp[2]) + " particles in " + str(temp[5]) +  " dimensions"  
...
def create_table_non_interacting(method, filedir, n_particles, n_dims, iterations):
    ...
    if method == "bf":
        method_full_name += "Metropolis Brute-force"
        cap = "Optimal energy, error and time for different network architectures using " + method_full_name + " on a non-interacting system with " + str(n_particles) + " particles in " + str(n_dims) + " dimensions. Using $\sigma^2 = $" + temp[11] + ", MC-iterations = " + temp[-1].split(".")[0] + " and MC step-length = " + temp[15] 
    ...
    if method == "gs":
        method_full_name += "Gibbs Sampling"
        cap = "Optimal energy, error and time for different network architectures using " + method_full_name + " on a non-interacting system with " + str(n_particles) + " particles in " + str(n_dims) + " dimensions. Using $\sigma^2 = $" + temp[11] + " and MC-iterations = " + temp[-1].split(".")[0]  
